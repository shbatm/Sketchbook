...
void loop(){
  ...
  /*see if we don't recieve anything i just go and refresh the screen and write, the data to it*/
    ...
    /*lets check if we got a clear then clear the stack and buffer after it :) so to speak restart the calculator.*/
    /*why !strcmp(); ? because if i don't it return 99 and that is true, and when clear in buffer returns 0... 
      not verry intuitive but yea... */
    if(!strcmp("clear",Buffer)){
      Serial.println("recieved a clear! ");
  ...
  if(currentMillis - previousMillis > interval){
    ...
    int i = 0;
    while(stack[i]!='\0'){
      lcd.print((unsigned long)stack[i]);
      i++;
    }
  }
}
...
long pop(){
  ...
  if(stackIndex < STACKSIZE){
    ...
    pointer by one.*/
    unsigned long item = stack[--stackIndex];
    return item;
  }
  else{
    /*print the informational message that something might be going wrong.*/
