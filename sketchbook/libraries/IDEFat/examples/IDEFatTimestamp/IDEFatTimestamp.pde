 ...
 */
#include <IDEFat.h>
#include <IDEFatUtil.h> // use PgmPrint
/*
 ...
 * See IDEFile::dateTimeCallback() for usage.
 */
 ...
 */
void printTimestamps(IDEFile& f) {
  dir_t d;
  ...
  PgmPrint("Access: ");
  f.printFatDate(d.lastAccesIDEate);
  Serial.println();
}
...
void setup(void) {
  ...
  IDEFile::remove(&root, "CALLBACK.TXT");
  IDEFile::remove(&root, "DEFAULT.TXT");
  IDEFile::remove(&root, "STAMP.TXT");
   ...
   * IDEFile::dateTimeCallbackCancel()
   */
  // set date time callback function
  IDEFile::dateTimeCallback(dateTime);
