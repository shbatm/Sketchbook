...
void setup() {
  //need this delay for screen voltage rise, has to do with IOlcd.h library
  delay(200);
  ...
  //Serial.begin(9600);
  //print2("here");
  //going to clear var[] buf[] and buffer[]; i notice this fixes memmory issue;also strcp large amounts of data 
  //is going to make your program slow and use more ram.
  clearBuffer();
  clearVar();
  clearbuf();
  delay(1000);
...
const int varlen = 100;
int var[varlen],CurrentLineNumber,quoted;
const int bufLen_ = 100;
char buf[bufLen_],F[2];//size of buf might be reducible
...
void loop() {
        ...
        while(1) {
               ...
               //#endif
                ...
                case 'R': // RUN
                        callp=callstk;
                        for(i=0;i<varlen;i++) var[i]=0;//reinitializes all variables to 0
